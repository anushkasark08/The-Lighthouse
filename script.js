...
function renderReviews() {
  ...
  allReviews.forEach(review => {
    ...
    const name = String(review.name || 'Guest').slice(0, 80);
    const text = String(review.text || '').slice(0, 500);
    const date = String(review.date || '').slice(0, 40);
    ...
    details.append(reviewName, reviewDate);
    author.append(avatar, details);
    card.append(stars, reviewText, author);
    grid.append(card);
  });
}
...
if (reviewForm) {
  reviewForm.addEventListener('submit', function(e) {
    ...
    const newReview = {
      id: Date.now(),
      name: document.getElementById('review-name').value.trim().slice(0, 80),
