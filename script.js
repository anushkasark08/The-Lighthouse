...
// Reuses the existing triggerFilter()/menuSearch logic above instead of
// duplicating it, so filter state (active category + cuisine) stays in sync.
const menuSearchClear = document.getElementById("menuSearchClear");

if (menuSearch && menuSearchClear) {
  const toggleMenuSearchClear = () => {
    menuSearchClear.hidden = menuSearch.value.length === 0;
  };

  // Runs in addition to the existing "input" listener registered above;
  // does not replace or interfere with it.
  menuSearch.addEventListener("input", toggleMenuSearchClear);

  menuSearchClear.addEventListener("click", () => {
    menuSearch.value = "";
    toggleMenuSearchClear();
    triggerFilter();
    menuSearch.focus();
  });

  // Set correct initial state (e.g. on page reload with a pre-filled value)
  toggleMenuSearchClear();
}

// ── "Clean Filter" Button ─────────────────────────
// Resets search text + category filter + cuisine dropdown + veg/non-veg
// filter back to defaults, by reusing the existing click handlers/logic
// above instead of duplicating any filtering code.
...
if (cleanFilterBtn) {
  cleanFilterBtn.addEventListener("click", () => {
    ...
    // Reset category filter ("All") — reuses the existing filterBtns click logic
    document.querySelector('.filter-btn[data-filter="all"]')?.click();

    // Reset cuisine dropdown, if present
    if (cuisineDropdown) {
      cuisineDropdown.value = "all";
    }

    // Reset veg/non-veg filter ("All Items") — reuses the existing diet-btn click logic
    document.querySelector('.diet-btn[data-diet="all"]')?.click();

    // Ensure everything is back in sync
    triggerFilter();
  });
}
